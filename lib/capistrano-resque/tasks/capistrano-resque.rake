      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            ...
            within current_path do
              execute %{cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE="#{queue}" PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 INTERVAL=#{fetch(:interval)} #{"environment" if fetch(:resque_environment_task)}}, :rvm, `cat .ruby-version`.strip, :do, :bundle, :exec, :rake, "resque:work"
            end
            worker_id += 1
          end
        end

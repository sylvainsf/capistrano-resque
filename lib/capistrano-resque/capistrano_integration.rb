            ...
            for_each_workers do |role, workers|
              ...
              workers.each_pair do |queue, number_of_workers|
                ...
                number_of_workers.times do
                  ...
                  run("cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE=#{queue} PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 #{fetch(:bundle_cmd, "bundle")} exec rake environment resque:work >> #{shared_path}/log/resque.log 2>&1 &", :roles => role, :shell => "bash")
                  worker_id += 1
                end
              end
            end
